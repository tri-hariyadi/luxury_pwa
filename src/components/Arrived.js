...
const Arrived = ({ items }) => {
  const currencyFormat = item => {
    ...
    let priceFormat = split[0].substr(0, rest);
        ...
        <div className="z-10 overflow-hidden">
          <div className="flex -mx-4 flex-row relative">
            {items && items.map((item, idx) => {
              return (
                <div className="px-4 relative card group" key={`arrived-${idx}`}>
                  <div
                  ...
                  <span className="">IDR {currencyFormat(item.price)}</span>
